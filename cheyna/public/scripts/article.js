...
(function (module) {
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map(article => article.body.match(/\b\w+/g).length).reduce((accumulator, currentValue) => accumulator + currentValue);
  };

  Article.allAuthors = () => {
    return Article.all.map(article => article.author).reduce((numAuthors, name) => {
      if(numAuthors.indexOf(name) === -1) numAuthors.push(name);
      return numAuthors;
    }, []);
  };
